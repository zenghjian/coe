...
def pc_normalize(pc, faces):
    ...
    area = face_area(pc, faces)
    sqrt_area = torch.sqrt(sum(area))

    # print(f'sqrt_area: {sqrt_area:.3f}')
    pc /= sqrt_area
...
def vertex_normal(verts, faces, n_neighbors=30):
    ...
    if faces is None: # point cloud
        _, neigh_inds = find_knn(verts, verts, n_neighbors, omit_diagonal=True, method='brute')
        neigh_inds_np = torch2np(neigh_inds)
        neigh_points = verts_np[neigh_inds_np, :]
...
def build_grad_point_cloud(verts, frames, n_neighbors=30):
    ...
    edges = np.stack((edge_inds_from, neigh_inds_np.flatten()))
...
def compute_wks_autoscale(evals, evecs, mass, n_descr=122, subsample_step=1, n_eig=128):
    feats = []
